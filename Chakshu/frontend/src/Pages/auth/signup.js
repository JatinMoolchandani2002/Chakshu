...
export default function Signup(){
    ...
    let initialImage = {
        userProfileImage: "profileImage.png"
    }
    const [image, setImage] = useState(initialImage)
    const states = stateArray.states
    const [form, setForm] = useState({userName:'', password:'', firstName:'', lastName:'', locality:'', city:'', state:'', pincode:'', aadharNum:'', contactNum:''})
    const [errorList, setErrorList] = useState({
        userNameError:'', passwordError:'', firstNameError:'', lastNameError:'',
        pincodeError:'', contactNumError:'', aadharNumError:''
    })
    ...
    const handleConfirm = ()=>{
        ...
        let config = {
            method: 'post',
            url: 'http://localhost:4000/api/auth/userSignUp',
            data : formData
        };
    ...
    const handleFormSubmit = async()=>{
        ...
        const firstNameRegex = /^.{3,15}$/
        const lastNameRegex = /^.{3,15}$/
        let flag = 0
        ...
        if(!firstNameRegex.test(form.firstName)){
            flag = 1
            const errorMessage = 'First Name must contain at least 3 characters'
            setErrorList(errorList=>({...errorList, firstNameError:errorMessage}))
        }
        if(!lastNameRegex.test(form.lastName)){
            flag = 1
            const errorMessage = 'Last Name must contain at least 3 characters'
            setErrorList(errorList=>({...errorList, lastNameError:errorMessage}))
        }
        if(form.aadharNum.length !== 12){
            flag = 1
            const errorMessage = 'Aadhar Number must be of 12 characters'
            setErrorList(errorList=>({...errorList, aadharNumError:errorMessage}))
        }
    ...
    return(
        <div className='h-full'>
            ...
            <div className='h-max flex flex-col justify-start select-none'>
                <div className='h-[6vh]' onClick={()=>navigate('/')}>
                    <AiFillHome className='text-[30px] mx-6 my-3 hover:scale-105 cursor-pointer text-gray-800'/>
                </div>
                <div className='min-h-[100vh] max-h-max flex items-center z-10 overflow-y-scroll py-8'>
                    <div className='w-[90vw] md:w-1/2 flex flex-col items-center justify-start mx-auto bg-white opacity-100 border-t-[6px] border-gray-800 shadow-xl rounded-lg h-max gap-6 py-8'>
                        ...
                        <div className='my-0 ml-10 md:ml-0 md:flex md:flex-row md:items-center md:justify-center md:gap-8 w-full md:w-9/12'>
                            <div className='mt-3 md:my-0 w-full md:w-9/12 md:flex md:flex-col md:items-start md:justify-center md:gap-2'>
                                <h1 className='font-[Poppins] font-medium text-xs md:text-sm text-black'>First Name</h1>
                                <input type="text" name='firstName' placeholder='Jatin' value={form.firstName} onChange={handleFormChange} autoComplete='off' className='mt-1 font-[Poppins] text-[11px] md:text-base placeholder:text-[#c5bfbf] w-[80%] md:w-full rounded-lg shadow-md focus:outline-none py-2 px-4 focus:ring-1 ring-gray-400'></input>
                            </div>
                        
                            <div className='mt-3 md:my-0 w-full md:w-9/12 md:flex md:flex-col md:items-start md:justify-center md:gap-2'>
                                <h1 className='font-[Poppins] font-medium text-xs md:text-sm text-black'>Last Name</h1>
                                <div className='flex flex-row w-full items-center'>
                                    <input type="text" name='lastName' placeholder='Mehra' value={form.lastName} onChange={handleFormChange} autoComplete='off' className='mt-1 font-[Poppins] text-[11px] md:text-base placeholder:text-[#c5bfbf] w-[80%] md:w-full rounded-lg shadow-md focus:outline-none py-2 px-4 focus:ring-1 ring-gray-400'></input>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className='my-0 ml-10 md:ml-0 md:flex md:flex-row md:items-center md:justify-center md:gap-8 w-full md:w-9/12'>
                            ...
                            <div className='mt-3 md:my-0 w-full md:w-9/12 md:flex md:flex-col md:items-start md:justify-center md:gap-2'>
                                <h1 className='font-[Poppins] font-medium text-xs md:text-sm text-black'>Aadhar UID</h1>
                                <div className='flex flex-row w-full items-center'>
                                    <input type="text" name='aadharNum' placeholder='458765982365' value={form.aadharNum} onChange={handleFormChange} autoComplete='off' className='mt-1 font-[Poppins] text-[11px] md:text-base placeholder:text-[#c5bfbf] w-[80%] md:w-full rounded-lg shadow-md focus:outline-none py-2 px-4 focus:ring-1 ring-gray-400'></input>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="w-9/12 items-center justify-center">
                            <div className='w-1/2 h-[250px] mx-auto bg-white rounded-md'>
                                <div className='flex flex-row justify-center items-center w-full h-full mx-auto overflow-hidden'>
                                    <img src={image.userProfileImage} alt="Uploaded Profile" className="rounded-md border-2 border-white"></img>
                                </div>
                            </div>
                        ...
                        <h1 onClick={()=>{navigate('/login')}} className='font-[Poppins] font-medium text-sm text-gray-500 mt-2 w-5/12 text-center hover:text-gray-800 cursor-pointer'>Already Registered? Login</h1>
                    </div>
                </div>
            </div>
